...
import {isGraphQLEnabled} from "@/utils/feature-flags";
...
// SVG icons for different community types
...
function getIconForCommunityType(type: string): string {
  switch (type) {
...
export async function createCommunitiesPanel(courseCode: string, dataUrl?: string, iconUrl?: string): Promise<HTMLElement> {
  const wrapper = document.createElement("div");
  wrapper.id = "wgu-ext-community-panel-wrapper";

  // Create the expansion panel structure matching WGU's style
  wrapper.innerHTML = `
    <mat-expansion-panel class="mat-expansion-panel mat-elevation-z0 mat-expansion-panel-animations-enabled mat-expanded mat-expansion-panel-spacing">
      <mat-expansion-panel-header role="button" class="mat-expansion-panel-header mat-focus-indicator mat-expansion-toggle-indicator-after mat-expanded" id="wgu-ext-community-panel-${courseCode}-header" tabindex="0" aria-controls="wgu-ext-community-child-${courseCode}" aria-expanded="true" aria-disabled="false" style="height: 40px;cursor: default;border-radius: 5px 5px 0 0;">
        <span class="mat-content">
          <img src="${iconUrl || "icon-128.png"}" style="height: 25px; margin-right: 10px;">
          <mat-panel-title class="mat-expansion-panel-header-title">
            <span class="accordion-title">Outside Community</span>
          </mat-panel-title>
        </span>
      </mat-expansion-panel-header>
      <div class="mat-expansion-panel-content-wrapper" style="visibility: visible;">
        <div role="region" class="mat-expansion-panel-content" id="wgu-ext-community-child-${courseCode}" aria-labelledby="wgu-ext-community-panel-${courseCode}-header">
          <div class="mat-expansion-panel-body">
            <div class="expansion-content" style="border-radius: 0 0 5px 5px; padding: 15px;">
              <div id="community-content">Loading community links...</div>
            </div>
          </div>
        </div>
      </div>
...
async function loadFromUrl(dataUrl: string, wrapper: HTMLElement, courseCode: string) {
  ...
  console.log(`Attempting to fetch community data from: ${dataUrl}`);
