...
export const ingestWguConnectCollection = onRequest({
  ...
  // Validate body
  const errs: string[] = [];
  if (!body || body.type !== "wgu_connect") errs.push("type must be 'wgu_connect'");
  ...
  if (!body?.fingerprint || !isFingerprintValid(body.fingerprint)) errs.push("fingerprint is required and must be hex 32..64");
